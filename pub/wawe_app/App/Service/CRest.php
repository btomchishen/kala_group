...
class CRest
{
    ...
    public static function installApp()
    {
        ...
        if(!empty($_REQUEST[ 'auth' ]) && $_REQUEST[ 'event' ] == 'ONAPPINSTALL')
        {
            $_REQUEST[ 'auth' ]['portal_domain'] = $_REQUEST[ 'auth' ]['domain'];
        ...
        elseif($_REQUEST['PLACEMENT'] == 'DEFAULT')
        {
            ...
            $result['install'] = static::setAppSettings(
                [
                    ...
                    'protocol' => empty($_REQUEST['PROTOCOL']) ? '' : $_REQUEST['PROTOCOL'],
                    'placement' => empty($_REQUEST['PLACEMENT']) ? '' : $_REQUEST['PLACEMENT'],
                    'placement_options' => empty($_REQUEST['PLACEMENT_OPTIONS']) ? '' : $_REQUEST['PLACEMENT_OPTIONS'],
                    ...
                    'portal_domain' => htmlspecialchars($_REQUEST['DOMAIN']),
    ...
    public static function checkAccessParams($domain, $access_token)
    {
        $result = false;
        ...
        $result = static::callCurl($arPost, $arSettings);
     ...
     *
     */
    protected static function callCurl($arParams, $arSettings = array())
    {
        ...
        if($arSettings !== false)
        {
            ...
            try
            {
                ...
                $nowMicroSec = hrtime(true);
                $microSecSaved = static::getDelayMSec();
                if($microSecSaved !== false) {
                    $eta = $nowMicroSec - $microSecSaved;
                    $mSec = $eta/1e+6;
                    if($mSec < 500) {
                        $sleep = intval((500 - $mSec));
                        $sleep = $sleep * 1000;
                        usleep($sleep);
                    }
                }
                ...
                static::saveNSeconds(hrtime(true));
    ...
    {
        $return = false;
        if(empty($arSettings['portal_domain'])) {
            if(!empty($_REQUEST['DOMAIN'])) {
                $arSettings['portal_domain'] = $_REQUEST['DOMAIN'];
            }
        }
        $correct = static::checkAccessParams($arSettings['portal_domain'], $arSettings['access_token']);
    ...
    protected static function getSettingData()
    {
        ...
        if(!empty($_REQUEST['DOMAIN'])) {
            $return = Portal::getByDomain($_REQUEST['DOMAIN']);

            if(empty($return['use_local_install'])) {
                ...
                if(!empty($return['c_rest_client_id'])) {
                    $return['C_REST_CLIENT_ID'] = $return['c_rest_client_id'];
                }
                if(!empty($return['c_rest_client_secret'])) {
                    $return['C_REST_CLIENT_SECRET'] = $return['c_rest_client_secret'];
                }
            }
        }
        return $return;
    ...
    {
        $params = Portal::prepare($arSettings);
        if(!empty($params['portal_domain']) &&
        !empty($params['access_token']) &&
        !empty($params['refresh_token']) &&
        !empty($params['application_token'])
        ) {
            $res = Portal::getByDomain($params['portal_domain']);
            if(empty($res)) {
                Portal::add($params);
            } else {
                Portal::update($params);
            }
        }
    ...
    public static function saveNSeconds($nSec)
    {
        if(!empty($_REQUEST['DOMAIN'])) {
            $portal = $_REQUEST['DOMAIN'];
            $path = realpath(__DIR__ . '/../..');
            $path .= '/logs/'.$portal.'/delay/';
            if (!file_exists($path))
            {
                @mkdir($path, 0775, true);
            }
            $path .= 'm_sec.txt';
            file_put_contents($path, $nSec, LOCK_EX);
        }
    }

    public static function getDelayMSec()
    {
        $result = false;
        if(!empty($_REQUEST['DOMAIN'])) {
